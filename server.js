...
// Accept the project URL only (for example https://project.supabase.co).
// Older setup notes used the /rest/v1 URL, so normalize either form safely.
...
app.get('/api/transactions', async (req, res) => {
  try {
    const response = await fetch(`${SUPABASE_URL}/rest/v1/transactions?select=*&order=date.desc,created_at.desc`, {
      headers: sbHeaders()
    });
